...
RUN go build -gcflags="${SKAFFOLD_GO_GCFLAGS}" -o /go/src/hipstershop/recommendationservice .
...
COPY --from=builder /go/src/hipstershop/recommendationservice /hipstershop/recommendationservice
