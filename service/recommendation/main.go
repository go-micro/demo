...
import (
	...
	"github.com/go-micro/demo/recommendation/config"
	"github.com/go-micro/demo/recommendation/handler"
...
func main() {
	...
	recommendation := &handler.RecommendationService{
		ProductCatalogService: pb.NewProductCatalogService(cfg.ProductCatalogService, client),
	}
	if err := pb.RegisterRecommendationServiceHandler(srv.Server(), recommendation); err != nil {
		logger.Fatal(err)
	}
